...
Mat nmx(Mat img_edge,Mat img_angle){
   Mat img_out=img_edge;
   for(int i=0;i<img_out.rows;i++){
      for(int j=0;j<img_out.cols;j++){
         //angle等于0
         if(img_angle.at<uchar>(i,j)==0){
            if(i==0){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i+1,j)){img_edge.at<uchar>(i,j)=0;}
            }
            else if(i==img_out.rows-1){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i-1,j)){img_edge.at<uchar>(i,j)=0;}
            }
            else {
               if(img_edge.at<uchar>(i,j)!=fmax(img_edge.at<uchar>(i,j),fmax(img_edge.at<uchar>(i,j+1),img_edge.at<uchar>(i,j-1)))){img_edge.at<uchar>(i,j)=0;}
            }
         }
         //angle等于45
         if(img_angle.at<uchar>(i,j)==45){
            if(i==0||j==img_out.cols-1){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i+1,j-1)){img_edge.at<uchar>(i,j)=0;}
            }
            else if(i==img_out.rows-1||j==0){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i-1,j+1)){img_edge.at<uchar>(i,j)=0;}
            }
            else {
               if(img_edge.at<uchar>(i,j)!=fmax(img_edge.at<uchar>(i,j),fmax(img_edge.at<uchar>(i-1,j+1),img_edge.at<uchar>(i+1,j-1)))){img_edge.at<uchar>(i,j)=0;}
            }
         }
         //angle等于90
         if(img_angle.at<uchar>(i,j)==90){
            if(j==0){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i,j+1)){img_edge.at<uchar>(i,j)=0;}
            }
            else if(j==img_out.cols-1){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i,j-1)){img_edge.at<uchar>(i,j)=0;}
            }
            else {
               if(img_edge.at<uchar>(i,j)!=fmax(img_edge.at<uchar>(i,j),fmax(img_edge.at<uchar>(i,j+1),img_edge.at<uchar>(i,j-1)))){img_edge.at<uchar>(i,j)=0;}
         ...
         //angle等于135
         if(img_angle.at<uchar>(i,j)==135){
            if(i==0||j==0){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i+1,j+1)){img_edge.at<uchar>(i,j)=0;}
            }
            else if(i==img_out.rows-1||j==img_out.cols-1){
               if(img_edge.at<uchar>(i,j)<img_edge.at<uchar>(i-1,j-1)){img_edge.at<uchar>(i,j)=0;}
            }
            else {
               if(img_edge.at<uchar>(i,j)!=fmax(img_edge.at<uchar>(i,j),fmax(img_edge.at<uchar>(i-1,j-1),img_edge.at<uchar>(i+1,j+1)))){img_edge.at<uchar>(i,j)=0;}
...
//设定阈值
Mat trsd(Mat img,int high_trsd,int low_trsd){
   Mat img_out=Mat::zeros(img.rows,img.cols,CV_8UC1);
   for(int i=0;i<img_out.rows;i++){
      for(int j=0;j<img_out.cols;j++){
         ...
         else if(img.at<uchar>(i,j)>low_trsd){
            for(int x=-1;x<2;x++){
               for(int y=-1;y<2;y++){
                  if(i!=0&&j!=0&&i!=img_out.rows-1&&j!=img_out.cols-1){
                     if(img.at<uchar>(i+x,j+y)>high_trsd){
                        img_out.at<uchar>(i,j)=255;
                     }
                  }
               }
...
int main(void)
{
   ...
   Mat edge_img=edge_get(Sobel_smk(Gaosi(srcImage),1),Sobel_smk(Gaosi(srcImage),0));
   Mat angle_img=angle_get(Sobel_smk(Gaosi(srcImage),1),Sobel_smk(Gaosi(srcImage),0));
   Mat canny_img=trsd(nmx(edge_img,angle_img),100,20);
   
   imwrite("answer_43.jpg",canny_img);
   waitKey(0);
   return 0;
}
